...
export class MI2 extends EventEmitter implements IBackend {
    ...
    if (procEnv) {
      ...
      for (const key in procEnv) {
        if (procEnv.hasOwnProperty(key)) {
          if (procEnv === null) delete env[key];
  ...
  load(
    ...
    if (!nativePath.isAbsolute(target)) target = nativePath.join(cwd, target);
  ...
  async openMainSSHConnection(
    ...
    if (args.forwardX11) {
      sshConn.on("x11", (info, accept, reject) => {
    ...
    sshConn.exec(sshCMD, execArgs, (err, stream) => {
      if (err) {
  ...
  //     this.conn2 = new Client();
  //     var test = function () {
  //       this.log("stderr", "READYYYYYYYY ");
  //     };
  //     this.sshConn.on("ready", test);
  //     this.sshConn.on("ready-to-use", test);
  //     this.sshConn.once("ready-to-use", test);
  ...
  //     var authHandler = (methodsLeft, partialSuccess, callback) => {
  //       this.log(
  //         "stderr",
  //         "MethodLeft:" + methodsLeft + " partialSuccess:" + partialSuccess
  //       );
  //       var methods = [
  //         "publickey",
  //         "hostbased",
  ...
  //         for (var j = 0; j < methodsLeft.length; ++j) {
  //           if (methods[i] == methodsLeft[j]) {
  //             this.log("stderr", "Try authentication method: " + methods[i]);
  //             return callback(methods[i]);
  //           }
  //         }
  //       }
  //       callback(false);
  ...
  //       debug: (line) => this.log("stderr", "ssh log: " + line),
  //       tryKeyboard: true,
  //       readyTimeout: 200000, // milliseconds
  ...
  //       debug: (line) => this.log("stderr", "(proxy) ssh log: " + line),
  //       tryKeyboard: true,
  //       readyTimeout: 200000, // miliseconds
  ...
  //         this.log(
  //           "stderr",
  //           "SSH key file '" + args.keyfile + "' does not exist!"
  //         );
  //         this.emit("quit");
  //         reject();
  //         return;
  ...
  //     if (args.proxyUseAgent) {
  //       proxyConnection.agent = process.env.SSH_AUTH_SOCK;
  //     } else if (args.proxyKeyfile) {
  //       if (proxyConnection.host !== undefined) {
  //         if (require("fs").existsSync(args.proxyKeyfile)) {
  //           proxyConnection.identity = args.proxyKeyfile;
  //           proxyConnection.privateKey = require("fs").readFileSync(
  //             args.proxyKeyfile
  //           );
  //         } else {
  //           this.log(
  //             "stderr",
  //             "Proxy SSH key file '" + args.proxyKeyfile + "' does not exist!"
  //           );
  //           this.emit("quit");
  //           reject();
  //           return;
  ...
  //       this.log("stderr", "First SSH connection fail" + err.toString());
  //       this.emit("quit");
  //       reject();
  //     });
  //     this.sshConn.on("end", () => {
  //       this.log("stderr", "First SSH connection end");
  //       this.emit("quit");
  //       reject();
  //     });
  //     this.sshConn.on("close", () => {
  //       this.log("stderr", "First SSH connection close");
  //       this.emit("quit");
  //       reject();
  //     });
  //     this.sshConn.on(
  ...
  //         this.log(
  //           "stderr",
  //           "First connection need keyboard:" +
  //           name +
  //           "-" +
  //           instructions +
  //           "-" +
  ...
  //       }
  //     );
  //     this.sshConn.on("banner", (msg, lang) => {
  //       this.log("stderr", "First connection show banner:" + msg + "-" + lang);
  //     });
  //     this.sshConn.on("continue", () => {
  //       this.log("stderr", "First connection can accept more requests");
  //     });
  ...
  //       this.sshConn
  //         .on("ready", function () {
  //           this.log("stderr", "Second SSH connection ready");
  //           this.openMainSSHConnection(
  //             this.sshConn,
  //             args,
  //             connectionArgs,
  //             procArgs,
  ...
  //         })
  //         .on("error", (err) => {
  //           this.log("stderr", "Second SSH connection fail" + err.toString());
  //           this.emit("quit");
  //           reject();
  //         });

  //       this.conn2.on("ready", function () {
  //         this.log("stderr", "First SSH connection ready");
  //         // sshConn.forwardOut("127.0.0.1", 12345, connectionArgs.host, connectionArgs.port, function (err, stream) {
  //         // 	if (err) {
  //         // 		this.log("stderr", "forwardOut error:" + err);
  //         // 		this.emit("quit");
  //         // 		return sshConn.end();
  //         // 	}
  //         // 	connectionArgs.sock = stream;
  //         // 	conn2.connect(connectionArgs);
  //         // });
  //       });

  //       this.conn2.connect(proxyConnection);
  //     } else {
  //       // this.sshConn.on("ready", function () {
  //       //   setTimeout(() => {
  //       //     // this.openMainSSHConnection(this.sshConn, args, procArgs, cwd, attach, target, reject, resolve, null);
  //       //   }, 10000);
  //       //   this.log("stderr", "First SSH connection ready");
  //       //   this.sshConn.exec("uptime", function (err, stream) {
  //       //     if (err) throw err;
  //       //     stream
  ...
  //       //         console.log("stderr", "STDOUT: " + data);
  //       //       })
  ...
  //       this.sshConn.connect(connectionArgs);
  //     }
  ...
  //     // 			if (err) {
  //     // 				this.log("stderr", "Could not run " + this.application + " over ssh!" + err.toString());
  //     // 				this.emit("quit");
  //     // 				reject();
  //     // 				return;
  //     // 			}
  ...
  //     // this.sshConn.connect().then(() => {
  //     // 	this.log("stderr", "Running " + this.application + " over ssh...");
  //     // 	const execArgs: any = {};
  //     // 	if (args.forwardX11) {
  //     // 		execArgs.x11 = {
  //     // 			single: false,
  ...
  ssh(args: SSHArguments, cwd: string, target: string, procArgs: string, separateConsole: string, attach: boolean): Thenable<any> {
    return new Promise((resolve, reject) => {
      ...
      const connectionArgs: any = {
        ...
        debug: (line) => this.log("stderr", "ssh log: " + line),
        // tryKeyboard: true,
        readyTimeout: 300000, // miliseconds
      ...
      if (args.useAgent) {
        ...
        if (require("fs").existsSync(args.keyfile)) {
          ...
          this.log("stderr", "SSH key file '" + args.keyfile + "' does not exist!");
          this.emit("quit");
          reject();
          return;
        }
      ...
      if (args.proxyUseAgent) {
        ...
        if (proxyConnection.host !== undefined) {
          if (require("fs").existsSync(args.proxyKeyfile)) {
            proxyConnection.identity = args.proxyKeyfile;
            // proxyConnection.privateKey = require('fs').readFileSync(args.proxyKeyfile);
          } else {
            this.log("stderr", "Proxy SSH key file '" + args.proxyKeyfile + "' does not exist!");
            this.emit("quit");
            reject();
            return;
          }
      ...
      this.log("stderr", "SSHHost " + connectionArgs.username + "@" + connectionArgs.host + ":" + connectionArgs.port);
      if (proxyConnection.host !== undefined) {
        this.log("stderr", "Through proxy " + proxyConnection.username + "@" + proxyConnection.host + ":" + proxyConnection.port);
        this.sshConn = new ssh2_promise([proxyConnection, connectionArgs], true);
      } else {
        this.sshConn = new ssh2_promise(connectionArgs, true);
      }
      ...
      this.sshConn.on("ssh", (status, _, payload) => {
        this.log("stderr", "First SSH connection: " + status + " | " + payload);
        this.emit("quit");
        reject();
      });
      this.sshConn.on("ssh:disconnect", (_, payload) => {
        this.log("stderr", "First SSH connection end " + payload);
        this.emit("quit");
        reject();
      });
      this.sshConn.on("ssh:beforedisconnect", (_, payload) => {
        this.log("stderr", "First SSH connection close " + payload);
        this.emit("quit");
        reject();
      });
      this.sshConn.on("keyboard-interactive", (name, instructions, lang, prompts, finish) => {
        this.log("stderr", "First connection need keyboard:" + name + "-" + instructions + "-" + lang + "-" + prompts);
        finish(['\n']);
      });
      this.sshConn.on("ssh:beforeconnect", (_, payload) => {
        this.log("stderr", "Before connect:" + payload);
      });
      this.sshConn.on("ssh:connect", (_, payload) => {
        this.log("stderr", "Connect " + payload);
      });
      ...
      this.sshConn.connect().then(() => {
        ...
        if (attach)
          sshCMD += " -p " + target;
        this.log("stderr", "Run command:" + sshCMD);
        this.sshConn.exec(sshCMD, execArgs, (err, stream) => {
          if (err) {
            this.log("stderr", "Could not run " + this.application + " over ssh!" + err.toString());
            this.emit("quit");
    ...
    return new Promise((resolve, reject) => {
      this.once("ui-break-done", () => {
        ...
        this.sendCommand("exec-run").then((info) => {
          if (info.resultRecords.resultClass == "running") resolve();
          else reject();
        }, reject);
      });
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      if (breakpoint.countCondition) {
        if (breakpoint.countCondition[0] == ">")
          location +=
            "-i " + numRegex.exec(breakpoint.countCondition.substr(1))[0] + " ";
        else {
  ...
  clearBreakPoints(): Thenable<any> {
    ...
    return new Promise((resolve, reject) => {
      this.sendCommand("break-delete").then(
        (result) => {
          if (result.resultRecords.resultClass == "done") {
  ...
  async getStack(maxLevels: number, thread: number): Promise<Stack[]> {
    ...
    return stack.map((element) => {
      ...
      if (file) {
        if (this.isSSH) file = posix.normalize(file);
        else file = nativePath.normalize(file);
      }
  ...
  async varListChildren(name: string): Promise<VariableObject[]> {
    ...
    const omg: VariableObject[] = children.map(
      (child) => new VariableObject(child[1])
    );
    return omg;
  }
